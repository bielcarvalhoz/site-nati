...
it('filters between built and academic projects', () => {
  render(<Projects />)
  expect(builtCard()).toBeInTheDocument()
  expect(academicCard()).not.toBeInTheDocument()

  fireEvent.click(screen.getByRole('button', { name: 'Acadêmicos' }))
...
it('opens a project dialog with context and solution, then closes it', () => {
  render(<Projects />)
  ...
  expect(within(dialog).getByText('Projeto')).toBeInTheDocument()
